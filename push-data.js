...
if (!snap.exists()) {
  await setDoc(ref, { curso: raw.curso ?? "", festivos: raw.festivos ?? [], profesores: raw.profesores ?? [], ...sections });
  console.log("✔ Documento creado en Firestore con", SECTIONS.map(s => `${Object.keys(sections[s]).length} ${s}`).join(", "));
} else {
  const current = snap.data();
  const payload = { curso: raw.curso ?? "", festivos: raw.festivos ?? [], profesores: raw.profesores ?? [] };
